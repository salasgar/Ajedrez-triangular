#!/usr/bin/env node
// Una sola partida 'rps' a nivel 8, vuelca TODOS los hallazgos de captura
// gratis ignorada a disco para diseccionarlos.
const fs = require('fs');
const vm = require('vm');
  ...
  while (!gameEnded() && plies < 60) {
    ...
    if (capturaGratis && !(mv.from === capturaGratis.from && mv.to === capturaGratis.to)) {
      hallazgos.push({
        ...
        piezaCapturada: board.get(capturaGratis.to).type,
        piezaCapturadora: board.get(capturaGratis.from).type,
        nPiezas: board.size,
      });
    }
...
`);
fs.writeFileSync('/private/tmp/claude-501/-Users-salasgar-Documents-git-Ajedrez-triangular/2645ba0b-b4b2-461f-8671-b7d7192b9188/scratchpad/hallazgos-13j.json', JSON.stringify(r, null, 2));
console.log('plies:', r.totalPlies, 'hallazgos:', r.hallazgos.length);
console.log(JSON.stringify(r.hallazgos.slice(0, 15), null, 2));

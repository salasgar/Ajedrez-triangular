...
Actualizada: 2026-08-26 14:20 — decisión de Juan Luis: la arena mide SOLO `rps-rey`;
`rpsls-rey` se APLAZA (no se aparca) hasta que `rps-rey` esté optimizada, y entonces
tendrá su propia pasada de arena partiendo de los valores ganadores.
...
## Qué hay que hacer
1. Con `entrenamiento/arena-motor.js` (tarea 17), medir SOLO en `rps-rey`
   (decisión de Juan Luis del 26-8-2026, cabecera):
